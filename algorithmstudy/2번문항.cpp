...
bool bfs(vector<string> drum, vector<vector<bool>> ch, int startX, int startY) {
	...
	while (!q.empty()) {
		...
		if (xx < 0 || yy < 0 || xx >= drum.size()+1 || yy >= drum.size()) continue;
		if (ch[xx][yy]) continue;
		if (drum[xx][yy] == 'E') return true;
		q.push({ xx,yy });
		ch[xx][yy] = true;
	}
	return false;
...
int solution(vector<string> drum) {
	...
	int answer = 0;
	vector<vector<bool>> ch = vector<vector<bool>>(drum.size(), vector<bool>(drum.size()+1, 0));
